...
def add_ngrok_header(response):
    """Bypass ngrok browser interstitial warning page."""
...
def get_history_data():
    ...
    if history_df_full is None:
        ...
        # CSV timestamps are in DD-MM-YYYY HH:MM format — must use dayfirst=True
        history_df_full['Timestamp'] = pd.to_datetime(
            history_df_full['Timestamp'], dayfirst=True
        )
        gc.collect() 
...
def run_forecast_logic(req_start, temp_offset=0):
    ...
    # Normalize to pd.Timestamp so comparisons always work regardless of
    # whether req_start is datetime.datetime or pd.Timestamp

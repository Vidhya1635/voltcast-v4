    ...
    def load(self):
        ...
        if repo_id and not already_synced:
            ...
            try:
                ...
                print("[OK] Cloud weights synchronized.")
            except Exception as e:
                print(f"[WARN] Sync skipped: {e}")
        elif already_synced:
            print("[*] Using cached weights (Turbo Load).")
        else:
            print("[INFO] No HF_REPO_ID found. Using local paths.")
        ...
        print("[OK] Scalers ready...")
        gc.collect()

        # 2. Load XGBoost
        self.xgb_model = joblib.load(XGB_MODEL_PATH)
        print("[OK] XGBoost ready...")
        gc.collect()
        ...
        if is_cloud:
            print("[CLOUD] Running in 'Lite Mode' (1-Model Ensemble) to save RAM.")
        ...
        for i in range(ensemble_limit):
            ...
            print(f"[OK] DL Model {i+1}/{ensemble_limit} ready...")
        ...
        self.loaded = True
        print(f"[OK] Engine Sync Complete ({'Lite' if is_cloud else 'Full'} Mode).")
        gc.collect()

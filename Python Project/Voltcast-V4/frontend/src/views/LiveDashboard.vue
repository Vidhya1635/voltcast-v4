<template>
  <div class="live-dashboard fade-in" :style="forecastData && forecastData.forecast[0] ? getWeatherBgStyle(forecastData.forecast[0].weather_code) : {}">
...
const initChart = () => {
  ...
  const option = {
    ...
    tooltip: { 
      ...
      formatter: (params) => {
        let tip = `<div style="padding:4px; font-family: Inter, sans-serif;">
          <b style="color: #94a3b8; font-size: 11px; text-transform: uppercase;">${params[0].axisValue.replace('T', ' ').substring(0, 16)}</b><br/>
    ...
    xAxis: {
      ...
      data: timestamps,
      axisLabel: {
        formatter: (val) => val.substring(5, 16).replace('T', ' '),
        interval: 'auto',
        color: '#000000',
        fontWeight: 'bold',
        fontSize: 12
  ...
  // Force resize after paint so flex container is fully measured
...
.stat-value.success { color: #15803d; } /* Darker green for bold contrast */
.stat-value.accent { color: #be123c; }  /* Darker red for bold contrast */
.stat-sub { font-size: 0.8rem; color: #334155; font-weight: 600; }
...
.echart { flex: 1; width: 100%; min-height: 400px; display: block; }

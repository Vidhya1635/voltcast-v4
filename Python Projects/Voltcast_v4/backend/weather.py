...
def fetch_weather_forecast(start_date, hours=168):
    ...
    for city, coords in WEATHER_CITIES.items():
        try:
            ...
            if df.isna().all().any():
                print(f"  [WARN] {city} weather API returned nulls. Using seasonal fallback.")
                weather_data[city] = _seasonal_fallback(city, start_date, hours)
            else:
                weather_data[city] = df
                print(f"  [OK] {city}: {len(df)} hours fetched")

        except Exception as e:
            print(f"  [WARN] {city} weather API failed: {e}. Using seasonal fallback.")
            weather_data[city] = _seasonal_fallback(city, start_date, hours)
...
def fetch_historical_weather(start_date, hours=168):
    ...
    for city, coords in WEATHER_CITIES.items():
        try:
            ...
            weather_data[city] = df
            print(f"  [OK] {city}: {len(df)} hours (historical)")

        except Exception as e:
            print(f"  [WARN] {city} historical API failed: {e}. Using seasonal fallback.")
            weather_data[city] = _seasonal_fallback(city, start_date, hours)
